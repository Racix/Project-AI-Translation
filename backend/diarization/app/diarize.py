...
def convert_to_wav(file_path: str, output_path: str):
    ...
    subprocess.call(
        ["ffmpeg", "-hide_banner", "-loglevel warning", "-i", file_path, output_path]
    )
...
def configurations(
    wav_path: str, domain: str, rttm: str | None, speakers: int
) -> OmegaConf:
    # Configuration yaml file
    DOMAIN_TYPE = (
        domain  # Can be meeting or telephonic based on domain type of the audio file
    )
    ...
    config.diarizer.oracle_vad = (
        False  # ----> ORACLE VAD Sätt till false om vi inte vill ha RTTM!!!
    )
